...
- **HASmartCharge.Backend.OCPP** — self-contained OCPP 1.6J central system (transport, session manager, message router). **Telemetry-first by design**: all inbound transactions are auto-accepted (no id-tag whitelist), and outbound commands are limited to UnlockConnector, ChangeAvailability, ChangeConfiguration/GetConfiguration, TriggerMessage (on-connect config push), and **SetChargingProfile** (a flat current cap in A, `ChargerControl.SetChargingCurrentLimitAsync`). Do NOT add RemoteStart/Stop — charging start/stop stays in Home Assistant (plan.md §1). SetChargingProfile only caps delivered current; it never starts or stops a transaction. The original "no charging profiles" lock was reversed 2026-07-12 to add the dashboard charge-power slider.
  - **Developer escape hatch (added 2026-08-04):** `POST /api/charger/ocpp/call {action, payload}` (Settings → Developer tab) sends **any** OCPP action straight through `ICommandSender`, deliberately bypassing `IChargerControl`. The "no RemoteStart/Stop" rule above still binds `IChargerControl` and the orchestrator — automatic charging control stays in Home Assistant. This endpoint is a manual diagnostic tool only: nothing in the app calls it, the frontend confirms before sending, and no code path may depend on it.
  - **Live frame log:** every raw frame is logged at exactly one choke point — `WebSocketConnection.SendAsync`/`ReceiveAsync` — because `ChargePointSession.SendCommandAsync` writes directly to the connection, so hooks in `OcppConnectionOrchestrator` miss all server-initiated CALLs. `OcppRawLog.Append` writes the (optional) diagnostic file and always raises `FrameObserved`; `OcppFrameLogBuffer` (Backend, singleton) keeps the last 50 and pushes them over SignalR (`OcppLogHub`, `/api/hubs/ocpp-log`) to the developer tab. `Publish` runs on the OCPP path: never let it throw, block, or await the broadcast.
  - **Charge-power slider:** dashboard slider works in **kW** → `POST /api/charger/power {kw}` → clamps to `ChargerSettings.ChargePowerMin/MaxKw`, converts kW→amps with `A = W / (PhaseCount × SupplyVoltage)` (rounded down to 0.1 A so it never overshoots), sends a `TxDefaultProfile`/`Relative` SetChargingProfile in **amps** (unit `A`, `numberPhases = PhaseCount`), and persists `ChargePowerSetpointKw` only when the charger replies `Accepted`. The kW→A conversion is deliberate: most OCPP 1.6 chargers cap *current*, not power. Min/max/voltage/phases are edited on the charger settings tab; the setpoint is owned by the power endpoint (the settings PUT must not touch it). **Alternate path (added 2026-08-15):** `ChargerSettings.ChargePowerControlMode = "Configuration"` makes the same endpoint write `ChargePowerConfigurationKey` (e.g. `USER_PMAX`) via ChangeConfiguration instead, with the setpoint converted to `ChargePowerConfigurationUnit` (A/mA/W/kW) by `Core/Charging/ChargePowerUnits.TryFormat` — always rounded down, trailing `.0` trimmed. `RebootRequired` counts as accepted in that mode. Reuses the `ChargingProfile`/`ChargingSchedule` types in `CommonTypes.cs`, whose optional fields are `[JsonIgnore(WhenWritingNull)]` so strict chargers don't reject a profile carrying `null` optionals. Needs charger smart-charging support.
...
- **HASmartCharge.Frontend** — React 19 + Vite + TS + TanStack Query + recharts + Tailwind v4. Pattern: `src/types/*` mirror API DTOs, `src/api/*` one module per domain over `api/client.ts`, `src/hooks/*` TanStack Query wrappers. Pages: Dashboard, Schedule, Settings (tabs), History. `PriceChart` highlights selected charge hours. The only realtime path is `@microsoft/signalr` in `hooks/useOcppFrameLog.ts` — the Vite `/api` proxy needs `ws: true` or the hub silently degrades to long-polling in dev.
